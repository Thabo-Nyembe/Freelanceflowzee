...
CREATE TABLE IF NOT EXISTS ai_collection_images (
    collection_id UUID  ON DELETE CASCADE,
    image_id UUID  ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS ai_content_variations (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    original_content_id UUID  ON DELETE CASCADE,
    user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS client_reviews (
    ...
    template_id UUID  ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS collab_document_comments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  session_id UUID  ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS community_likes (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    post_id UUID  ON DELETE CASCADE,
    comment_id UUID  ON DELETE CASCADE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    CHECK (
...
CREATE TABLE IF NOT EXISTS crm_activities (
  ...
  description TEXT,
  contact_id UUID  ON DELETE CASCADE,
  deal_id UUID  ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS email_agent_approvals (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  response_id UUID  ON DELETE CASCADE,
  message_id UUID  ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS file_metadata (
    ...
    hash TEXT NOT NULL,
    provider_id UUID NOT NULL  ON DELETE CASCADE,
    storage_tier_id UUID  ON DELETE SET NULL,
    storage_path TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS kb_article_feedback (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  article_id UUID  ON DELETE CASCADE,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS kb_articles (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
  category_id UUID  ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS kb_bookmarks (
  ...
  -- Bookmark Target
  article_id UUID  ON DELETE CASCADE,
  video_id UUID  ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS kb_categories (
  ...
  -- Hierarchy
  parent_id UUID  ON DELETE SET NULL,
  sort_order INTEGER DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS kb_search_queries (
  ...
  -- Results Clicked
  clicked_article_id UUID  ON DELETE SET NULL,
  clicked_video_id UUID  ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS report_filters (
  ...
  widget_id UUID  ON DELETE CASCADE,
  field TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS support_conversations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  agent_id UUID  ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS video_folders (
  ...
  -- Hierarchy
  parent_folder_id UUID  ON DELETE CASCADE,
  path TEXT,
