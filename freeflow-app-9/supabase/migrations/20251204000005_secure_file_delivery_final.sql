-- ============================================================================
-- Secure File Delivery System with Escrow Integration - FINAL VERSION
-- ============================================================================
...
DROP TABLE IF EXISTS file_delivery_recipients CASCADE;
DROP TABLE IF EXISTS file_access_logs CASCADE;
DROP TABLE IF EXISTS file_download_transactions CASCADE;
DROP TABLE IF EXISTS secure_file_deliveries CASCADE;
DROP TABLE IF EXISTS wasabi_storage_config CASCADE;
DROP TABLE IF EXISTS gallery_collections CASCADE;

-- ============================================================================
-- TABLES
-- ============================================================================

-- Gallery Collections Table (Create FIRST)
...
CREATE TABLE secure_file_deliveries (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
-- File Delivery Recipients Table
CREATE TABLE file_delivery_recipients (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  delivery_id UUID NOT NULL REFERENCES secure_file_deliveries(id) ON DELETE CASCADE,
...
CREATE TABLE wasabi_storage_config (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
-- File Download Transactions Table
CREATE TABLE file_download_transactions (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  delivery_id UUID NOT NULL REFERENCES secure_file_deliveries(id) ON DELETE CASCADE,
...
-- ============================================================================
-- ADD FOREIGN KEY CONSTRAINTS FOR ESCROW
-- ============================================================================
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    ...
    SET file_count = (
      SELECT COUNT(*) FROM collection_files WHERE collection_id = OLD.collection_id
    ),
    updated_at = NOW()
  ...
  ELSE
    UPDATE gallery_collections
    SET file_count = (
      SELECT COUNT(*) FROM collection_files WHERE collection_id = NEW.collection_id
    ),
    updated_at = NOW()
