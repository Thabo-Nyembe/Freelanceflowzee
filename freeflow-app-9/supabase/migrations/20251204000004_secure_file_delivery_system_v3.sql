-- ============================================================================
-- Secure File Delivery System with Escrow Integration - V3 (Fixed Version)
-- ============================================================================
...
-- Gallery Collections Table (Create FIRST - needed for foreign keys)
CREATE TABLE IF NOT EXISTS gallery_collections (
  ...
  CONSTRAINT positive_payment_gallery CHECK (payment_amount IS NULL OR payment_amount >= 0)
);
...
CREATE TABLE IF NOT EXISTS secure_file_deliveries (
  ...
  payment_amount DECIMAL(10, 2),
  payment_currency TEXT DEFAULT 'USD',
  escrow_deposit_id UUID,  -- Will add FK after checking if escrow_deposits exists
  milestone_id UUID,
  ...
  CONSTRAINT positive_payment_delivery CHECK (payment_amount IS NULL OR payment_amount >= 0),
...
-- Add foreign key for escrow_deposits if table exists
DO $$
BEGIN
  ...
  IF EXISTS (
    ...
    IF NOT EXISTS (
      ...
      WHERE constraint_name = 'fk_escrow_deposit'
      AND table_name = 'secure_file_deliveries'
    ) THEN
      ALTER TABLE secure_file_deliveries
      ADD CONSTRAINT fk_escrow_deposit
      FOREIGN KEY (escrow_deposit_id)
      REFERENCES escrow_deposits(id)
      ON DELETE SET NULL;
    END IF;

    -- Check if constraint doesn't already exist for transactions
    IF NOT EXISTS (
      SELECT 1
      FROM information_schema.table_constraints
      WHERE constraint_name = 'fk_transaction_escrow_deposit'
      AND table_name = 'file_download_transactions'
    ) THEN
      ALTER TABLE file_download_transactions
      ADD CONSTRAINT fk_transaction_escrow_deposit
      FOREIGN KEY (escrow_deposit_id)
      REFERENCES escrow_deposits(id)
      ON DELETE SET NULL;
    END IF;
...
DROP POLICY IF EXISTS "Users can view own deliveries" ON secure_file_deliveries;
DROP POLICY IF EXISTS "Users can create deliveries" ON secure_file_deliveries;
DROP POLICY IF EXISTS "Users can update own deliveries" ON secure_file_deliveries;
DROP POLICY IF EXISTS "Users can delete own deliveries" ON secure_file_deliveries;
...
DROP POLICY IF EXISTS "Users can view own access logs" ON file_access_logs;
DROP POLICY IF EXISTS "Anyone can insert access logs" ON file_access_logs;
...
DROP POLICY IF EXISTS "Users can view public collections or own collections" ON gallery_collections;
DROP POLICY IF EXISTS "Users can create collections" ON gallery_collections;
DROP POLICY IF EXISTS "Users can update own collections" ON gallery_collections;
DROP POLICY IF EXISTS "Users can delete own collections" ON gallery_collections;
...
DROP POLICY IF EXISTS "Users can view own storage config" ON wasabi_storage_config;
DROP POLICY IF EXISTS "Users can create own storage config" ON wasabi_storage_config;
DROP POLICY IF EXISTS "Users can update own storage config" ON wasabi_storage_config;
...
DROP POLICY IF EXISTS "Users can view own transactions" ON file_download_transactions;
DROP POLICY IF EXISTS "Anyone can create transactions" ON file_download_transactions;
...
CREATE INDEX IF NOT EXISTS idx_secure_file_deliveries_owner ON secure_file_deliveries(owner_id);
CREATE INDEX IF NOT EXISTS idx_secure_file_deliveries_recipient ON secure_file_deliveries(recipient_id);
CREATE INDEX IF NOT EXISTS idx_secure_file_deliveries_status ON secure_file_deliveries(status);
CREATE INDEX IF NOT EXISTS idx_secure_file_deliveries_escrow ON secure_file_deliveries(escrow_deposit_id);
CREATE INDEX IF NOT EXISTS idx_secure_file_deliveries_expires ON secure_file_deliveries(expires_at);
CREATE INDEX IF NOT EXISTS idx_secure_file_deliveries_gallery ON secure_file_deliveries(gallery_category);

-- File Access Logs Indexes
CREATE INDEX IF NOT EXISTS idx_file_access_logs_delivery ON file_access_logs(delivery_id);
CREATE INDEX IF NOT EXISTS idx_file_access_logs_user ON file_access_logs(accessed_by_id);
CREATE INDEX IF NOT EXISTS idx_file_access_logs_timestamp ON file_access_logs(accessed_at DESC);
CREATE INDEX IF NOT EXISTS idx_file_access_logs_action ON file_access_logs(action);

-- Gallery Collections Indexes
CREATE INDEX IF NOT EXISTS idx_gallery_collections_owner ON gallery_collections(owner_id);
CREATE INDEX IF NOT EXISTS idx_gallery_collections_slug ON gallery_collections(slug);
CREATE INDEX IF NOT EXISTS idx_gallery_collections_public ON gallery_collections(is_public) WHERE is_public = true;
CREATE INDEX IF NOT EXISTS idx_gallery_collections_featured ON gallery_collections(is_featured) WHERE is_featured = true;

-- Collection Files Indexes
CREATE INDEX IF NOT EXISTS idx_collection_files_collection ON collection_files(collection_id);
CREATE INDEX IF NOT EXISTS idx_collection_files_delivery ON collection_files(delivery_id);

-- File Download Transactions Indexes
CREATE INDEX IF NOT EXISTS idx_file_download_transactions_delivery ON file_download_transactions(delivery_id);
CREATE INDEX IF NOT EXISTS idx_file_download_transactions_buyer ON file_download_transactions(buyer_id);
CREATE INDEX IF NOT EXISTS idx_file_download_transactions_escrow ON file_download_transactions(escrow_deposit_id);
CREATE INDEX IF NOT EXISTS idx_file_download_transactions_status ON file_download_transactions(status);
