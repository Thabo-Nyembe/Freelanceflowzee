-- ============================================================================
-- FIX REMAINING 400/401/406 ERRORS (v2 - with DROP IF EXISTS)
-- ============================================================================
...
CREATE POLICY "team_invitations_insert" ON team_invitations FOR INSERT WITH CHECK (auth.uid() = invited_by OR auth.uid() IS NOT NULL);
CREATE POLICY "team_invitations_update" ON team_invitations FOR UPDATE USING (auth.uid() = invited_by OR auth.uid() IS NOT NULL);
CREATE POLICY "team_invitations_delete" ON team_invitations FOR DELETE USING (auth.uid() = invited_by OR auth.uid() IS NOT NULL);
...
CREATE POLICY "ai_feature_usage_select" ON ai_feature_usage FOR SELECT USING (auth.uid() IS NOT NULL);
...
CREATE POLICY "user_metrics_aggregate_policy" ON user_metrics_aggregate FOR ALL USING (auth.uid() = user_id OR auth.uid() IS NOT NULL);
...
CREATE POLICY "growth_metrics_policy" ON growth_metrics FOR ALL USING (auth.uid() = user_id OR auth.uid() IS NOT NULL);
...
CREATE POLICY "user_analytics_policy" ON user_analytics FOR ALL USING (auth.uid() = user_id OR auth.uid() IS NOT NULL);
...
CREATE POLICY "analytics_revenue_policy" ON analytics_revenue FOR ALL USING (auth.uid() = user_id OR auth.uid() IS NOT NULL);
...
CREATE POLICY "notification_queue_policy" ON notification_queue FOR ALL USING (auth.uid() = user_id OR auth.uid() IS NOT NULL);
...
CREATE POLICY "digital_assets_policy" ON digital_assets FOR ALL USING (auth.uid() = user_id OR auth.uid() IS NOT NULL);
...
CREATE POLICY "crm_deals_policy" ON crm_deals FOR ALL USING (auth.uid() = user_id OR auth.uid() IS NOT NULL);
...
CREATE POLICY "crm_activities_policy" ON crm_activities FOR ALL USING (auth.uid() = user_id OR auth.uid() IS NOT NULL);
...
CREATE POLICY "crm_contacts_policy" ON crm_contacts FOR ALL USING (auth.uid() = user_id OR auth.uid() IS NOT NULL);
...
CREATE POLICY "automations_policy" ON automations FOR ALL USING (auth.uid() = user_id OR auth.uid() IS NOT NULL);
...
CREATE POLICY "reminder_recipients_policy" ON reminder_recipients FOR ALL USING (auth.uid() = user_id OR auth.uid() IS NOT NULL);
