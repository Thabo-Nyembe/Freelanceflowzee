...
interface ClientComment {
  ...
  userAvatar: string
  content: string
  type: 'inline' | 'voice' | 'screen' | 'text'
  fileType: 'figma' | 'video' | 'pdf' | 'text'
  position?: { x: number; y: number; elementId?: string }
  status: 'open' | 'resolved' | 'in_progress' | 'changes_required' | 'approved'
  priority: 'low' | 'medium' | 'high'
  category: 'design' | 'copy' | 'technical'
  createdAt: string
...
interface ProjectFile {
  id: string
  name: string
  type: 'figma' | 'video' | 'pdf' | 'text'
  currentVersion: FileVersion
  versions: FileVersion[]
  url: string
  thumbnail: string
  status: 'needs_review' | 'in_progress' | 'approved' | 'on_hold'
  approvalStatus: {
    overall: 'pending' | 'approved' | 'rejected'
    elements: Record<string, 'pending' | 'approved' | 'rejected'>
  }
...
interface ClientCollabState {
  ...
  viewMode: 'inline' | 'list'
  filterBy: 'all' | 'me' | 'unresolved'
...
const sampleFiles: ProjectFile[] = [
  {
    ...
    aiSummary: {
      ...
      actionItems: ['Increase footer font size for mobile'],
    }
  }
]
...
export function EnhancedClientCollaboration({
  ...
  const submitComment = async () => {
    const newComment: ClientComment = {
      ...
      fileType: state.activeFile?.type || 'figma',
        ...
        <CardHeader>
          <div className="flex justify-between items-center">
            <CardTitle className="flex items-center gap-3">
              ...
              <Badge className={getStatusColor((state.activeFile?.status === 'approved' ? 'approved' : 'open') as ClientComment['status'])}>
                ...
                {state.comments.map(
                  (comment: ClientComment) =>
                    comment.position && (
                      ...
                      >
                        <div
                          className={`w-6 h-6 rounded-full flex items-center justify-center text-white text-xs font-semibold shadow-lg hover:scale-110 transition-transform ${
                            comment.status === 'approved'
                              ? 'bg-green-500'
                              : comment.status === 'changes_required'
                              ? 'bg-red-500'
        ...
        <div className="col-span-4">
          <Card className="bg-white/70 backdrop-blur-sm border-slate-200/50">
            ...
            <CardContent className="space-y-4 max-h-96 overflow-y-auto">
              {state.comments.map((comment: ClientComment) => (
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      {comment.replies.length > 0 && (
                        <div className="ml-4 border-l-2 border-slate-200 pl-3 space-y-2">
                          {comment.replies.map((reply: ClientComment) => (
