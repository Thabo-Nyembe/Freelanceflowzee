...
// Configuration
const CONFIG = {
  projectRoot: '/Users/thabonyembe/Documents/freeflow-app-9',
...
const checkComponentErrors = async (componentPath) => {
  const fullPath = path.join(CONFIG.projectRoot, componentPath);
  log(`Analyzing component: ${componentPath}`);
  
  if (!fs.existsSync(fullPath)) {
    log(`Component file not found: ${fullPath}`, 'error');
    return {
      status: 'error',
...
const fixIconImports = (componentPath) => {
  const fullPath = path.join(CONFIG.projectRoot, componentPath);
  log(`Attempting to fix icon imports in ${componentPath}`);
  ...
  try {
    ...
    if (missingIcons.length === 0) {
      log(`No missing icon imports found in ${componentPath}`, 'success');
    ...
    log(`Fixed missing icon imports in ${componentPath}`, 'success');
    ...
    log(`Failed to fix icon imports in ${componentPath}: ${err.message}`, 'error');
    return {
      status: 'error',
      error: err.message
    };
...
const rollbackNavigationChanges = async () => {
  ...
  for (const [component, backup] of Object.entries(latestBackups)) {
    const fullComponentPath = path.join(CONFIG.projectRoot, component);
    const result = restoreBackup(backup, fullComponentPath);
    if (!result) {
      success = false;
    }
  }
...
const generateErrorReport = (results) => {
  const timestamp = new Date().toISOString().replace(/[:.]/g, '');
  const reportPath = path.join(CONFIG.projectRoot, `emergency-recovery-report-${timestamp}.json`);
...
const main = async () => {
  ...
  log('------------------------------------------------', 'info');
  log(`Project root: ${CONFIG.projectRoot}`, 'info');
