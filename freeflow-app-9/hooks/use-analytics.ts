...
interface PerformanceEventTimingWithProcessingStart extends PerformanceEntry {
  processingStart: number;
  startTime: number;
}

interface LayoutShiftWithHadRecentInput extends PerformanceEntry {
  value: number;
  ...
  private setupPerformanceTracking() {
    ...
    if ('PerformanceObserver' in window) {
      ...
      this.observePerformanceMetric('first-input', (entries) => {
        const firstEntry = entries[0] as PerformanceEventTimingWithProcessingStart;
      ...
      this.observePerformanceMetric('layout-shift', (entries) => {
        entries.forEach((entry: PerformanceEntry) => {
          const layoutShift = entry as LayoutShiftWithHadRecentInput;
