...
test.describe('Capture All Pages', () => {
  for (let i = 0; i < v2Pages.length; i++) {
    const pageName = v2Pages[i];
    const num = String(i + 1).padStart(3, '0');

    test(`${num} - ${pageName}`, async ({ page }) => {
      test.setTimeout(30000);

      await page.goto(`${BASE_URL}/dashboard/${pageName}`, { timeout: 20000 });
      ...
      const hasError = pageContent.includes('Error:') || pageContent.includes('Unknown error');

      if (hasError) {
        console.log(`⚠️ ${pageName} has error visible`);
      } else {
