          ...
          <TabsContent value="my-templates" className="space-y-6">
            ...
            <div className="grid grid-cols-4 gap-4">
              {[
                ...
                { icon: Copy, label: 'Duplicate', desc: 'Copy template', color: 'text-purple-500', action: () => { if (selectedTemplate) { toast.promise(new Promise(r => setTimeout(r, 800)), { loading: `Duplicating "${selectedTemplate.name}"...`, success: `"${selectedTemplate.name}" duplicated`, error: 'Failed to duplicate' }); } else { toast.warning('Select a template to duplicate') } } },
                { icon: Download, label: 'Export All', desc: 'Download all', color: 'text-cyan-500', action: () => setIsExportOpen(true) },
                { icon: FolderPlus, label: 'Organize', desc: 'Add to folder', color: 'text-orange-500', action: () => { if (selectedTemplate) { toast.promise(new Promise(r => setTimeout(r, 600)), { loading: `Organizing "${selectedTemplate.name}"...`, success: `"${selectedTemplate.name}" organized`, error: 'Failed to organize' }); } else { toast.warning('Select a template to organize') } } },
