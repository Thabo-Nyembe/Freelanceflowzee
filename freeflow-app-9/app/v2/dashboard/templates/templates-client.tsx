          ...
          <TabsContent value="my-templates" className="space-y-6">
            ...
            <div className="grid grid-cols-4 gap-4">
              {[
                ...
                { icon: Copy, label: 'Duplicate', desc: 'Copy template', color: 'text-purple-500', action: () => { if (selectedTemplate) { toast.success('Template Duplicated', { description: `"${selectedTemplate.name}" copied` }); } else { toast.warning('Select a template to duplicate') } } },
                { icon: Download, label: 'Export All', desc: 'Download all', color: 'text-cyan-500', action: () => setIsExportOpen(true) },
                { icon: FolderPlus, label: 'Organize', desc: 'Add to folder', color: 'text-orange-500', action: () => { if (selectedTemplate) { toast.success('Template Organized', { description: `"${selectedTemplate.name}" added to folder` }); } else { toast.warning('Select a template to organize') } } },
