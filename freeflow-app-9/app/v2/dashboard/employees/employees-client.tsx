              ...
              <CardContent className="p-0 divide-y divide-gray-100 dark:divide-gray-800">
                {mockTimeOffRequests.map(request => (
                  <div key={request.id} className="flex items-center gap-4 p-4 hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    {request.status === 'pending' && <div className="flex gap-2"><Button size="icon" variant="ghost" className="text-green-600" onClick={() => toast.promise(new Promise(r => setTimeout(r, 800)), { loading: 'Approving request...', success: 'Time off request approved', error: 'Failed to approve request' })}><CheckCircle className="h-4 w-4" /></Button><Button size="icon" variant="ghost" className="text-red-600" onClick={() => toast.promise(new Promise(r => setTimeout(r, 800)), { loading: 'Rejecting request...', success: 'Time off request rejected', error: 'Failed to reject request' })}><XCircle className="h-4 w-4" /></Button></div>}
                  </div>
                ))}
              </CardContent>
            </Card>
