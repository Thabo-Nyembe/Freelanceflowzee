...
export default function AnalyticsClient() {
  ...
  const { data: dbCohorts = [], isLoading: cohortsLoading } = useAnalyticsCohorts(userId || undefined)
  const { data: dbRevenue = [], isLoading: revenueLoading } = useAnalyticsRevenue(userId || undefined)
  ...
  const mockMetrics = dbMetrics
  const mockFunnels = dbFunnels
  const mockReports = dbReports
  ...
  const handleCreateFunnel = async () => {
    if (!userId) {
      toast.error('Operation Failed', { description: 'Please try again or contact support' })
      return
    }
  ...
  const handleNotifications = () => {
    ...
    toast.info('Notification Settings', { description: 'Configure your analytics alert preferences' })
  }
  ...
  'https://analytics.kazi.app/track.js?id='+i+dl;f.parentNode.insertBefore(j,f);
    ...
    try {
      ...
      toast.success('Tracking Code Copied', { description: 'Paste this script into your website header' })
    } catch (err) {
...
Report generated by Kazi Analytics
  ...
  const handleRefresh = async () => {
    setIsLoading(true)
    try {
      await Promise.all([refreshFunnels(), refreshReports(), refreshDashboards(), refreshMetrics()])
  ...
  // Key metrics for header cards - Using real data where available
  const getMetric = (name: string) => dbMetrics.find((m: any) => m.name === name) || {}
  const keyMetrics = [
    { label: 'Customers', value: (getMetric('Customers')?.value || 0).toLocaleString(), change: `${getMetric('Customers')?.changePercent > 0 ? '+' : ''}${getMetric('Customers')?.changePercent || 0}%`, positive: (getMetric('Customers')?.changePercent || 0) >= 0, icon: Users, gradient: 'from-indigo-500 to-indigo-600' },
    { label: 'MRR', value: `$${((getMetric('MRR')?.value || 0) / 1000).toFixed(0)}K`, change: `${getMetric('MRR')?.changePercent > 0 ? '+' : ''}${getMetric('MRR')?.changePercent || 0}%`, positive: (getMetric('MRR')?.changePercent || 0) >= 0, icon: DollarSign, gradient: 'from-emerald-500 to-emerald-600' },
    { label: 'Conversion', value: `${getMetric('Conversion')?.value || 0}%`, change: `${getMetric('Conversion')?.changePercent > 0 ? '+' : ''}${getMetric('Conversion')?.changePercent || 0}%`, positive: (getMetric('Conversion')?.changePercent || 0) >= 0, icon: Target, gradient: 'from-purple-500 to-purple-600' },
    { label: 'NPS', value: (getMetric('NPS')?.value || 0).toString(), change: `${getMetric('NPS')?.changePercent > 0 ? '+' : ''}${getMetric('NPS')?.changePercent || 0}%`, positive: (getMetric('NPS')?.changePercent || 0) >= 0, icon: TrendingUp, gradient: 'from-amber-500 to-amber-600' },
    { label: 'ARR', value: `$${((getMetric('ARR')?.value || 0) / 1000000).toFixed(1)}M`, change: `${getMetric('ARR')?.changePercent > 0 ? '+' : ''}${getMetric('ARR')?.changePercent || 0}%`, positive: (getMetric('ARR')?.changePercent || 0) >= 0, icon: Eye, gradient: 'from-blue-500 to-blue-600' },
    { label: 'Churn', value: `${getMetric('Churn Rate')?.value || 0}%`, change: `${getMetric('Churn Rate')?.changePercent > 0 ? '+' : ''}${getMetric('Churn Rate')?.changePercent || 0}%`, positive: (getMetric('Churn Rate')?.changePercent || 0) <= 0, icon: MousePointer, gradient: 'from-rose-500 to-rose-600' },
    { label: 'LTV:CAC', value: `${(getMetric('LTV:CAC')?.value || 0).toFixed(1)}x`, change: `${getMetric('LTV:CAC')?.changePercent > 0 ? '+' : ''}${getMetric('LTV:CAC')?.changePercent || 0}%`, positive: (getMetric('LTV:CAC')?.changePercent || 0) >= 0, icon: Zap, gradient: 'from-cyan-500 to-cyan-600' },
    { label: 'Enterprise', value: (getMetric('Enterprise')?.value || 0).toString(), change: `${getMetric('Enterprise')?.changePercent > 0 ? '+' : ''}${getMetric('Enterprise')?.changePercent || 0}%`, positive: (getMetric('Enterprise')?.changePercent || 0) >= 0, icon: Target, gradient: 'from-pink-500 to-pink-600' }
  ]

  // Realtime metrics - Using real data
  const getRealtimeValue = (metricName: string) => dbRealtimeMetrics.find((m: any) => m.metric === metricName)?.value || 0
  const realtimeMetrics = [
    { label: 'Active Users', value: getRealtimeValue('active_users'), icon: Users, trend: 12 },
    { label: 'Page Views/min', value: getRealtimeValue('page_views_per_min'), icon: Eye, trend: 8 },
    { label: 'AI Requests/min', value: getRealtimeValue('ai_requests_per_min'), icon: Activity, trend: 15 },
    { label: 'Conversions', value: getRealtimeValue('conversions'), icon: ShoppingCart, trend: 2 }
  ]

  // Calculate total revenue
  const totalRevenue = dbRevenue.reduce((acc: number, curr: any) => acc + (Number(curr.amount) || Number(curr.value) || 0), 0)
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 dark:bg-none dark:bg-gray-900 p-4 md:p-6 lg:p-8">
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <Button variant="ghost" className="bg-white/20 hover:bg-white/30 text-white border-0" onClick={handleExport} aria-label="Export data">
                ...
                <Button
                  ...
                  className={showInsights ? 'bg-white text-purple-600' : 'bg-white/20 hover:bg-white/30 text-white border-0'}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <TabsList className="bg-white dark:bg-gray-800 p-1 rounded-xl shadow-sm border h-auto flex-wrap">
          ...
          <TabsTrigger value="performance" className="data-[state=active]:bg-indigo-600 data-[state=active]:text-white rounded-lg px-4 py-2">
            <Activity className="h-4 w-4 mr-2" />
            Performance
          </TabsTrigger>
                  ...
                  <div className="space-y-2">
                    {Array.from({ length: 12 }, (_, i) => ({
                      ...
                      <div key={idx} className="flex items-center justify-between py-2 px-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                        <div className="flex items-center gap-2">
                          <div className={`h-2 w-2 rounded-full ${event.event === 'Purchase' ? 'bg-emerald-500' :
        ...
        <TabsContent value="performance" className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-500">Monitor system health and infrastructure metrics</p>
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {/* Summary Cards */}
            {['Uptime', 'Response Time', 'Error Rate', 'CPU Usage'].map((metric) => (
              <Card key={metric}>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Detailed system performance logs</CardDescription>
            </CardHeader>
            <CardContent>
              {perfLoading ? (
                ...
                <div className="space-y-4">
                  {performanceAnalytics.map((item: PerformanceAnalytic) => (
                    <div key={item.id} className="flex items-center justify-between border-b pb-4 last:border-0 last:pb-0">
                      <div>
                        <p className="font-medium">{item.metric_name}</p>
                        <p className="text-sm text-gray-500">{item.metric_category} • {item.environment}</p>
                      </div>
                      <div className="text-right">
                        <p className="font-bold">{item.current_value.toLocaleString()}</p>
                        <p className={`text-xs ${item.is_optimal ? 'text-emerald-500' : 'text-amber-500'}`}>
                          {item.is_optimal ? 'Optimal' : 'Needs Attention'}
                        </p>
                      </div>
                    </div>
                  ))}
