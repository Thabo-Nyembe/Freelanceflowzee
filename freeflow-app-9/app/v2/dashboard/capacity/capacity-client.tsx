...
export default function CapacityClient({ initialCapacity }: { initialCapacity: Capacity[] }) {
  ...
  const handleGenerateReport = () => {
    // Generate report based on reportForm settings
    let reportData: string
    const dateStr = new Date().toISOString().split('T')[0]

    if (reportForm.format === 'csv') {
      // CSV export based on report type
      ...
      reportData = [csvHeaders, ...csvRows].map(row => row.join(',')).join('\n')
      const blob = new Blob([reportData], { type: 'text/csv;charset=utf-8;' })
      const url = URL.createObjectURL(blob)
      const link = document.createElement('a')
      link.href = url
      link.download = `capacity-${reportForm.reportType}-report-${dateStr}.csv`
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
      URL.revokeObjectURL(url)
    } else if (reportForm.format === 'json') {
      // JSON export
      const jsonData = {
      ...
      const link = document.createElement('a')
      link.href = url
      link.download = `capacity-${reportForm.reportType}-report-${dateStr}.json`
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
      URL.revokeObjectURL(url)
    } else {
      // PDF format - generate text report
      reportData = `
CAPACITY REPORT
===============
...
Overbooked Members: ${stats.overbooked}
...
Report generated by Kazi Capacity Planning
      ...
      const link = document.createElement('a')
      link.href = url
      link.download = `capacity-${reportForm.reportType}-report-${dateStr}.txt`
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
      URL.revokeObjectURL(url)
    ...
    toast.success('Report generated', { description: `${reportForm.reportType} report downloaded as ${reportForm.format.toUpperCase()}` })
  ...
  const handleExportSchedule = () => {
    ...
    document.body.removeChild(link)
    URL.revokeObjectURL(url)
    toast.success('Schedule exported', { description: 'CSV file downloaded' })
  }
  ...
  const handleAddDepartment = () => {
    ...
    toast.success("Department added: " + newDepartmentName + " department has been created")
  ...
  const handleConnectIntegration = (name: string) => {
    toast.success('Connection initiated: connecting to ' + name + '...')
  }

  const handleRegenerateApiKey = () => {
    // Generate a new API key
    const newApiKey = `cap_${crypto.randomUUID().replace(/-/g, '').substring(0, 32)}`
    navigator.clipboard.writeText(newApiKey).then(() => {
      toast.success('API key regenerated', { description: 'New key copied to clipboard' })
    }).catch(() => {
      toast.success('API key regenerated', { description: `Key: ${newApiKey.substring(0, 12)}...` })
    })
  }
  ...
  const handleExportData = () => {
    ...
    const link = document.createElement('a')
    link.href = url
    link.download = `capacity-data-export-${new Date().toISOString().split('T')[0]}.json`
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
    URL.revokeObjectURL(url)
    toast.success('Data exported', { description: 'All capacity data downloaded as JSON' })
  }
        ...
        reader.onload = (event) => {
          try {
            ...
            if (file.name.endsWith('.json')) {
              ...
              toast.success('Data imported', { description: `Loaded ${data.teamMembers?.length || 0} members, ${data.projects?.length || 0} projects` })
            } else {
              const lines = content.split('\n')
              toast.success('Data imported', { description: `Loaded ${lines.length - 1} records from CSV` })
            }
          } catch (err) {
