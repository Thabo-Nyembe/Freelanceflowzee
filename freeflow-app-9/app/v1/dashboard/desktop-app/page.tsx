    ...
    if (os === 'macOS') {
      return (
        <div className="flex items-center gap-2 p-3">
          <button className="w-3 h-3 bg-red-500 rounded-full hover:bg-red-600" onClick={() => { logger.info('Minimizing window', { os, app }); setIsMinimized(true); toast.promise(new Promise(r => setTimeout(r, 500)), { loading: 'Minimizing window...', success: `Window minimized - ${app} - ${os}`, error: 'Failed to minimize window' }); }} data-testid="minimize-window-btn" />
          <button className="w-3 h-3 bg-yellow-500 rounded-full hover:bg-yellow-600" onClick={() => { logger.info(isMinimized ? 'Restoring window' : 'Minimizing window', { os, app, state: isMinimized ? 'minimized' : 'normal' }); const action = isMinimized ? 'restored' : 'minimized'; setIsMinimized(!isMinimized); toast.promise(new Promise(r => setTimeout(r, 500)), { loading: `${isMinimized ? 'Restoring' : 'Minimizing'} window...`, success: `Window ${action} - ${app} - ${os}`, error: 'Failed to toggle window' }); }} data-testid="toggle-minimize-btn" />
          <button className="w-3 h-3 bg-green-500 rounded-full hover:bg-green-600" onClick={() => { logger.info(isMaximized ? 'Restoring window' : 'Maximizing window', { os, app, state: isMaximized ? 'maximized' : 'normal' }); const action = isMaximized ? 'restored' : 'maximized'; setIsMaximized(!isMaximized); toast.promise(new Promise(r => setTimeout(r, 500)), { loading: `${isMaximized ? 'Restoring' : 'Maximizing'} window...`, success: `Window ${action} - ${app} - ${os}`, error: 'Failed to toggle window' }); }} data-testid="toggle-maximize-btn" />
        </div>
      )
    } else {
      return (
