...
const logger = createFeatureLogger('store-apps-actions')
...
export async function getStoreStats(): Promise<ActionResult<any>> {
  try {
    ...
    if (!apps) {
      return actionSuccess({ total: 0, installed: 0, available: 0, trial: 0, totalDownloads: 0, avgRating: 0 }, 'Store stats fetched successfully')
    }
