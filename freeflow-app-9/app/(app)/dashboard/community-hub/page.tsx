...
export default function CommunityHubPage() {
  ...
  // Empty typed arrays - data comes from Supabase
  const emptyMembers: CommunityMember[] = []

  const emptyPosts: CommunityPost[] = []

  const emptyEvents: CommunityEvent[] = []
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Set current user from Supabase data or first transformed member, otherwise null
        const currentUser = currentMemberResult.data ? {
          ...
          projects: [],
          tags: []
        } : (transformedMembers.length > 0 ? transformedMembers[0] : null)

        // Dispatch all data to reducer - use empty arrays if no data from Supabase
        ...
        // Use empty arrays on error - no mock data fallback
        dispatch({ type: 'SET_MEMBERS', payload: emptyMembers })
        dispatch({ type: 'SET_POSTS', payload: emptyPosts })
        dispatch({ type: 'SET_EVENTS', payload: emptyEvents })
        dispatch({ type: 'SET_GROUPS', payload: emptyGroups })
