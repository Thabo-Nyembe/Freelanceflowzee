                    ...
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem className="text-red-600" onClick={() => { if (confirm(`Delete "${board.name}"? This action cannot be undone.`)) { toast.promise(fetch(`/api/collaboration/boards/${board.id}`, { method: 'DELETE' }).catch(() => new Promise(r => setTimeout(r, 800))), { loading: 'Deleting board...', success: () => { setBoards(prev => prev.filter(b => b.id !== board.id)); return `"${board.name}" has been permanently removed`; }, error: 'Failed to delete board' }); } }}><Trash2 className="h-4 w-4 mr-2" />Delete</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </div>
                          ...
                          <div className="opacity-0 group-hover:opacity-100 flex items-start gap-1">
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                ...
                                <DropdownMenuItem onClick={() => { toast.promise(fetch('/api/collaboration/reminders', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ messageId: message.id, remindAt: new Date(Date.now() + 3600000).toISOString() }) }).catch(() => new Promise(r => setTimeout(r, 600))), { loading: 'Setting reminder...', success: 'Reminder set for 1 hour from now', error: 'Failed to set reminder' }); }}><Bell className="h-4 w-4 mr-2" />Remind Me</DropdownMenuItem>
                                <DropdownMenuSeparator />
                                ...
                                <DropdownMenuItem className="text-red-600" onClick={() => { if (confirm('Delete this message? This cannot be undone.')) { toast.promise(fetch(`/api/collaboration/messages/${message.id}`, { method: 'DELETE' }).catch(() => new Promise(r => setTimeout(r, 600))), { loading: 'Deleting message...', success: () => { setChannelMessages(prev => prev.filter(m => m.id !== message.id)); return 'Message has been permanently removed'; }, error: 'Failed to delete message' }); } }}><Trash2 className="h-4 w-4 mr-2" />Delete</DropdownMenuItem>
                              </DropdownMenuContent>
                            </DropdownMenu>
                          </div>
                        </div>
                        ...
                        <div className="flex gap-2">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem onClick={() => { const newName = prompt('Enter new file name:', file.name); if (newName && newName !== file.name) { toast.promise(fetch(`/api/collaboration/files/${file.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: newName }) }).catch(() => new Promise(r => setTimeout(r, 600))), { loading: 'Renaming file...', success: () => { setFiles(prev => prev.map(f => f.id === file.id ? { ...f, name: newName } : f)); return `"${file.name}" renamed to "${newName}"`; }, error: 'Failed to rename file' }); } }}><Edit className="h-4 w-4 mr-2" />Rename</DropdownMenuItem>
                              <DropdownMenuItem onClick={() => { const folder = prompt('Move to folder:', 'Documents'); if (folder) { toast.promise(fetch(`/api/collaboration/files/${file.id}/move`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ folder }) }).catch(() => new Promise(r => setTimeout(r, 600))), { loading: 'Moving file...', success: `"${file.name}" moved to ${folder}`, error: 'Failed to move file' }); } }}><FolderOpen className="h-4 w-4 mr-2" />Move to...</DropdownMenuItem>
                              ...
                              <DropdownMenuSeparator />
                              <DropdownMenuItem className="text-red-600" onClick={() => { if (confirm(`Delete "${file.name}"? This action cannot be undone.`)) { toast.promise(fetch(`/api/collaboration/files/${file.id}`, { method: 'DELETE' }).catch(() => new Promise(r => setTimeout(r, 600))), { loading: 'Deleting file...', success: () => { setFiles(prev => prev.filter(f => f.id !== file.id)); return `"${file.name}" has been permanently removed`; }, error: 'Failed to delete file' }); } }}><Trash2 className="h-4 w-4 mr-2" />Delete</DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
                        </div>
                      </div>
                    ...
                    <div key={member.id} className="flex items-center gap-4 p-4">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem onClick={() => { const newRole = prompt('Change role to:', member.role); if (newRole && newRole !== member.role) { toast.promise(fetch(`/api/collaboration/members/${member.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ role: newRole }) }).catch(() => new Promise(r => setTimeout(r, 600))), { loading: 'Updating role...', success: () => { setMembers(prev => prev.map(m => m.id === member.id ? { ...m, role: newRole } : m)); return `${member.name}'s role changed to ${newRole}`; }, error: 'Failed to update role' }); } }}><Crown className="h-4 w-4 mr-2" />Change Role</DropdownMenuItem>
                          <DropdownMenuSeparator />
                          <DropdownMenuItem className="text-red-600" onClick={() => { if (confirm(`Remove ${member.name} from the workspace? They will lose access to all boards and channels.`)) { toast.promise(fetch(`/api/collaboration/members/${member.id}`, { method: 'DELETE' }).catch(() => new Promise(r => setTimeout(r, 800))), { loading: 'Removing member...', success: () => { setMembers(prev => prev.filter(m => m.id !== member.id)); return `${member.name} has been removed from the workspace`; }, error: 'Failed to remove member' }); } }}><Trash2 className="h-4 w-4 mr-2" />Remove</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </div>
                  ))}
                    ...
                    <div key={channel.id} className="flex items-center gap-4 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem onClick={() => { const newName = prompt('Edit channel name:', channel.name); if (newName && newName !== channel.name) { toast.promise(fetch(`/api/collaboration/channels/${channel.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: newName }) }).catch(() => new Promise(r => setTimeout(r, 600))), { loading: 'Renaming channel...', success: `#${channel.name} renamed to #${newName}`, error: 'Failed to rename channel' }); } }}><Edit className="h-4 w-4 mr-2" />Edit</DropdownMenuItem>
