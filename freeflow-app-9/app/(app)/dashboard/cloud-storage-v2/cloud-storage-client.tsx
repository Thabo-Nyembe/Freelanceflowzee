...
const storageAIInsights: { id: string; type: 'success' | 'warning' | 'info'; title: string; description: string; priority: 'low' | 'medium' | 'high'; timestamp: string; category: string }[] = []
const storageCollaborators: { id: string; name: string; avatar: string; status: 'online' | 'offline' | 'away'; role: string }[] = []
const storagePredictions: { id: string; title: string; prediction: string; confidence: number; trend: 'up' | 'down' | 'stable'; impact: 'low' | 'medium' | 'high' }[] = []
const storageActivities: { id: string; user: string; action: string; target: string; timestamp: string; type: 'success' | 'info' | 'update' }[] = []
...
export default function CloudStorageClient() {
  ...
  const handleScan = async () => {
    toast.promise(
      refetch(),
      {
        loading: 'Scanning for file changes...',
        success: 'File scan completed',
          ...
          <TabsContent value="transfers" className="mt-6 space-y-6">
            ...
            <div className="bg-gradient-to-r from-orange-600 via-red-600 to-pink-600 rounded-2xl p-6 text-white">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-6">
                  <div className="text-center">
                    <p className="text-3xl font-bold">{transfers.filter(t => t.status === 'in_progress').length}</p>
