...
/**
 * Support V2 - Groundbreaking Customer Support Dashboard
 * Showcases support tickets and team performance with modern components
 */
export default function SupportV2() {
  const [selectedPriority, setSelectedPriority] = useState<'all' | 'urgent' | 'high' | 'normal'>('all')

  const stats = [
    { label: 'Open Tickets', value: '124', change: -12.5, icon: <MessageSquare className="w-5 h-5" /> },
    { label: 'Avg Response Time', value: '2.4h', change: -18.7, icon: <Clock className="w-5 h-5" /> },
    { label: 'Resolution Rate', value: '94%', change: 8.3, icon: <CheckCircle className="w-5 h-5" /> },
    { label: 'CSAT Score', value: '4.8', change: 12.5, icon: <Award className="w-5 h-5" /> }
  ]
  ...
  const tickets = [
    ...
    {
      id: 'TICK-2846',
      subject: 'Billing inquiry for annual plan',
      customer: 'David Kim',
      priority: 'normal',
      status: 'in_progress',
  ...
  const recentActivity = [
    { icon: <CheckCircle className="w-5 h-5" />, title: 'Ticket resolved', description: 'TICK-2843 marked as resolved', time: '10 minutes ago', status: 'success' as const },
    { icon: <MessageSquare className="w-5 h-5" />, title: 'New ticket created', description: 'TICK-2847 from Sarah Johnson', time: '2 hours ago', status: 'info' as const },
    ...
    { icon: <AlertCircle className="w-5 h-5" />, title: 'SLA warning', description: 'TICK-2844 approaching deadline', time: '5 hours ago', status: 'warning' as const }
  ]
    ...
    <div className="min-h-screen bg-gradient-to-br from-teal-50 via-cyan-50/30 to-blue-50/40 p-6">
      <div className="max-w-[1800px] mx-auto space-y-6">
        <div className="flex items-center justify-between">
          ...
          <GradientButton from="teal" to="cyan" onClick={() => console.log('New ticket')}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <BentoQuickAction icon={<MessageSquare />} title="Tickets" description="All tickets" onClick={() => console.log('Tickets')} />
          <BentoQuickAction icon={<Users />} title="Team" description="Agents" onClick={() => console.log('Team')} />
          <BentoQuickAction icon={<Award />} title="Satisfaction" description="Feedback" onClick={() => console.log('Satisfaction')} />
          <BentoQuickAction icon={<TrendingUp />} title="Analytics" description="Reports" onClick={() => console.log('Analytics')} />
        </div>
                  ...
                  <div key={ticket.id} className="p-4 rounded-xl border border-border bg-background hover:bg-muted/50 transition-colors">
                    ...
                    <div className="flex items-center justify-between pt-2 border-t">
                      ...
                      <div className="flex items-center gap-2">
                        <ModernButton variant="outline" size="sm" onClick={() => console.log('View', ticket.id)}>
                          View
                        </ModernButton>
                        <ModernButton variant="outline" size="sm" onClick={() => console.log('Reply', ticket.id)}>
